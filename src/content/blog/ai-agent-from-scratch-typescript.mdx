...
description: "How I built a real AI agent from scratch in TypeScript - no LangChain, just a loop, the OpenAI API, and the architecture lessons frameworks hide from you."
pubDate: "March 28 2026"
heroImage: "../../assets/heroes/ai-agent-from-scratch.webp"
heroAlt: "Architecture diagram for a TypeScript AI agent built without frameworks"
...
So I decided to build an agent from scratch - no LangChain, no CrewAI, no orchestration framework. Just TypeScript, the OpenAI API, and a handful of small libraries for things that aren't the point (persistence, spinners, env vars). The starting point was Scott Moss's [Build an AI Agent from Scratch](https://frontendmasters.com/courses/ai-agents-v2/) course on Frontend Masters - I followed its project skeleton, then rebuilt and extended it until I could reason about every line. Not as a learning exercise. As a foundation I can actually extend when things go wrong in production.
...
<ol class="agent-flow">
  <li>
    <span class="agent-flow__title">Persist the user's message</span>
    <span class="agent-flow__desc">to a running conversation history.</span>
  </li>
  <li>
    <span class="agent-flow__title">Send the full history to the LLM</span>
    <span class="agent-flow__desc">together with the set of available tools.</span>
  </li>
  <li>
    <span class="agent-flow__title">Recognize the right tool</span>
    <span class="agent-flow__desc">in this case, the DALL-E image generation tool.</span>
  </li>
  <li>
    <span class="agent-flow__title">Execute the tool call</span>
    <span class="agent-flow__desc">with a well-crafted prompt derived from the message.</span>
  </li>
  <li>
    <span class="agent-flow__title">Feed the result back</span>
    <span class="agent-flow__desc">return the image URL to the LLM as tool output.</span>
  </li>
  <li>
    <span class="agent-flow__title">Compose a final response</span>
    <span class="agent-flow__desc">let the LLM phrase it as natural language.</span>
  </li>
  <li>
    <span class="agent-flow__title">Print it to the terminal</span>
    <span class="agent-flow__desc">close the loop with the user.</span>
  </li>
</ol>
...
The code is intentionally minimal - ~200 lines across 10 files. Fork it, add a tool, break something, fix it.

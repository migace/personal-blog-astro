...
description: "Kiedy CPQ i model EAV naprawdę mają sens, kiedy są przerostem formy, i czemu tak często lądują w tej samej bazie kodu."
...
import EavExplorerIsland from "../../../components/EavExplorer/EavExplorerIsland.astro";
...
Ten kompromis można poczuć na własnej klawiaturze, zamiast wierzyć mi na słowo. Eksplorator poniżej wykonuje te same trzy zadania na obu modelach przechowywania: wczytanie jednej konfiguracji, filtrowanie po dwóch atrybutach i dodanie zupełnie nowego atrybutu. Przełączaj modele i patrz na generowany SQL oraz liczbę joinów - i zwróć uwagę, który model wymaga migracji przy trzecim zadaniu. Opóźnienia są symulowane przez `setTimeout`, liczby wierszy są poglądowe; uczciwa część to kształt SQL-a.
